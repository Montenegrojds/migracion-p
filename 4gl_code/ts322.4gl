...
# PROGRAMA: ts322.4gl
# VERSION : 5.1.0
# FECHA   : 02/09/97,27/03/98,03/07/98
# OBJETIVO: Deposito de Efectivo y Cheques del mismo banco
# AUTOR   : MDR
# COMPILAR: ts322.4gl ts000.4gl ts900.4gl ad800.4gl
...
GLOBALS "gbglobs.4gl"
		...
		g_tdes          LIKE cjtrn.cjtrnntra,
		g_ncja          LIKE cjmcj.cjmcjncja,
		g_acja          LIKE cjmcj.cjmcjncja,
                g_item          LIKE cntcn.cntcnitem,
	       	g_cctb          LIKE cntcn.cntcncctb,
		g_adic          LIKE cntcn.cntcnscaa,
                g_glosa         LIKE cntcn.cntcndesc,
		g_unec		LIKE cjmcj.cjmcjuneg, #U/N de caja
		g_unea		LIKE cjmcj.cjmcjuneg, #U/N anterior de caja
		g_uncc		SMALLINT,
                g_freg          LIKE cpmcp.cpmcpfreg,
...
MAIN DEFER INTERRUPT IF NOT f0000_open_database_gb000() THEN EXIT PROGRAM END IF
	LET g_mtflag = 0
        ...
        OPEN FORM ts322_01 FROM "ts322a"
        DISPLAY FORM ts322_01
	LET g_fpro = TODAY
...
--*     LET g_mgui = "N"        
        ...
        CALL f6050_buscar_empresa_ts322()
       #-----------------------------------------------------------------------
	LET g_nmod = "TESORERIA"
	LET g_titu = "DEP. EFECTIVO Y CHQS. MISMO BCO."
	LET g_vers = "5.1.12s"
--*	LET g_vers = "6.0.1"
	...
	IF NOT f1000_clave_gb000(t0.gbpmtnomb,t0.gbpmtcusr,96) THEN
	    ...
	    EXIT PROGRAM
	END IF
        CALL f0500_cursor_ts322()
        CALL f7000_crear_temporal_ad800()
        CALL f0250_declarar_puntero_ts322()
        CALL f0300_proceso_ts322()
END MAIN
...
FUNCTION f0250_declarar_puntero_ts322()
	DEFINE	s1	CHAR(500)
...
FUNCTION f0300_proceso_ts322()
	...
	WHILE TRUE
	CALL f5400_buscar_caja_ts322()
        CALL f6000_limpiar_menu_ts322()
	...
	IF int_flag THEN
	    RETURN
	END IF
        CALL f5000_buscar_registro_ts322()
       	MESSAGE " "
       	IF g_marca THEN
            MESSAGE " CTRL-Z para reimprimir comprobante"
       	END IF
        INPUT BY NAME t1.tsdepndoc,t1.tsdepftra,t1.tsdepcbco,t1.tsdepncta,
		      t1.tsdepcmon,t1.tsdeptcam,t1.tsdepiefe,
		      t1.tsdepgls1,t1.tsdepgls2,t1.tsdepgls3
                WITHOUT DEFAULTS
		ON KEY (CONTROL-C,INTERRUPT)
			MESSAGE " "
		        LET int_flag = TRUE 
		...
		ON KEY (CONTROL-E)
				...
				LET l_cdin = NULL
                                IF NOT f0330_validar_ts322() THEN
			        ...
			        CALL f0402_autorizacion_ad800
				         (g_user,96,2,t1.tsdepimpt,t1.tsdepcmon,
				          l_cdin,16,1,t1.tsdepftra,160,
					  t1.tsdepntra)
			    	          RETURNING g_uaut,g_flag
			        IF g_flag THEN
                                    IF f3000_borrar_ts322() THEN
			                LET int_flag = TRUE
		                        EXIT INPUT
			            END IF
			        END IF
		...
		ON KEY (CONTROL-V)
			    ...
			    NEXT FIELD tsdepcmon
                        END IF
                ON KEY (CONTROL-T)
                        IF g_marca AND t1.tsdepichq > 0 THEN
			    MESSAGE " "
			    ...
			    LET int_flag = FALSE
               	...
               	ON KEY (CONTROL-Z)
                       	IF g_marca THEN
			    CALL f8000_imprimir_comprobante_ts322()
                       	END IF
                BEFORE INPUT
			LET l_exit = FALSE
                        IF g_marca THEN
			    ...
			    END IF
                            ...
                            LET g_uncc = TRUE
                            IF NOT f5040_buscar_adhuu_ts322(t1.tsdepuneg) THEN
                                LET g_msj = "Usuario no habilitado en la U/N ",
                                      t1.tsdepuneg USING "<<<<"
                                CALL qx030_msgadvetencia_gb000(g_msj)      
                                LET g_uncc = FALSE
                            END IF
			    IF t1.tsdepiefe > 0 THEN
                                IF NOT f5040_buscar_adhuu_ts322(g_unea) THEN
                                    LET g_msj = "Usuario no habilitado en la U/N ",
                                          g_unea USING "<<"
                                    CALL qx030_msgadvetencia_gb000(g_msj)      
                                    LET g_uncc = FALSE
			    ...
			    #------------------------------------------------#
                            ...
                            CALL f5010_buscar_gbhtc_ts322(t1.tsdepftra)
			...
			IF t1.tsdepftra IS NULL THEN
			    ...
			    DISPLAY BY NAME t1.tsdepftra
			END IF
			DISPLAY BY NAME t1.tsdepimpt,t1.tsdepimpo
			  ...
			  #------------------------------------------------#
                ...
                AFTER FIELD tsdepcbco
                        IF t1.tsdepcbco IS NULL THEN
                            NEXT FIELD tsdepcbco
                        END IF
			CALL f5010_buscar_gbcon_ts322(11,t1.tsdepcbco)
			...
			DISPLAY BY NAME g_bank
                AFTER FIELD tsdepncta
                        IF t1.tsdepncta IS NULL THEN
                            NEXT FIELD tsdepncta
                        END IF
			IF NOT f5020_buscar_tsccb_ts322() THEN
			    NEXT FIELD tsdepncta
			END IF
			
                        IF NOT f5040_buscar_adhuu_ts322(t2.tsccbuneg) THEN
                            LET g_msj = "Usuario no habilitado en la U/N ",
                                  t2.tsccbuneg USING "<<<<"
                            CALL qx030_msgadvetencia_gb000(g_msj)      
                            NEXT FIELD tsdepncta
                        END IF
	                CALL f5010_buscar_gbcon_ts322(10,t2.tsccbcmon)
			...
			IF t1.tsdepcmon IS NULL THEN
			    NEXT FIELD tsdepcmon
			END IF
			CALL f5010_buscar_gbcon_ts322(10,t1.tsdepcmon)
			...
			CALL f0310_calcular_totales_ts322()
			...
			IF t1.tsdepichq > 0 THEN
				...
				END IF
			    END FOR
                            CALL f5030_buscar_tscha_ts322(p1[i].ncta,p1[i].nchq)
				      RETURNING g_flag
			...
			IF t1.tsdeptcam IS NULL AND
			   t1.tsdepcmon = t2.tsccbcmon THEN
			...
			END IF
                AFTER FIELD tsdeptcam
                        ...
                        END IF
			CALL f0310_calcular_totales_ts322()
		AFTER FIELD tsdepiefe
                        IF t1.tsdepiefe IS NULL OR t1.tsdepiefe < 0 THEN
			    LET t1.tsdepiefe = 0
                        END IF
			IF t1.tsdepiefe > 0 THEN
			    IF g_ncja IS NULL THEN
				    LET g_msj = "Usuario no es Cajero"
					CALL qx030_msgadvetencia_gb000(g_msj)
			        LET t1.tsdepiefe = 0
			        NEXT FIELD tsdepiefe
			    END IF
			    IF NOT f5040_buscar_adhuu_ts322(g_unec) THEN
                                LET g_msj = "Usuario no habilitado en la U/N ",
                                      g_unec USING "<<"
                    CALL qx030_msgadvetencia_gb000(g_msj)                  
			        LET t1.tsdepiefe = 0
			...
			CALL f0310_calcular_totales_ts322()
                        ...
                        LET l_fld = (l_key = fgl_keyval("ESC")) OR
                                    ...
                                    (l_key = fgl_keyval("UP")) 
                        IF NOT l_fld THEN
                            CALL f0400_proceso_cheques_ts322()
                        END IF
...
FUNCTION f0310_calcular_totales_ts322()
	LET t1.tsdepichq = 0
	FOR i=1 TO max_v1
	...
	END FOR
	LET t1.tsdepimpt = t1.tsdepiefe + t1.tsdepichq
...
FUNCTION f0400_proceso_cheques_ts322()
        ...
        INPUT ARRAY p1 WITHOUT DEFAULTS FROM s1.*
                ON KEY (CONTROL-C,INTERRUPT)
                        IF p1[l_curr].ncta IS NOT NULL THEN
                            IF p1[l_curr].nchq IS NULL THEN  
                                NEXT FIELD ncqh
                            END IF
                            IF f0410_repetidos_ts322(l_curr) THEN
			        NEXT FIELD nchq
			    END IF
                            IF NOT f5030_buscar_tscha_ts322
				        (p1[l_curr].ncta,p1[l_curr].nchq) THEN
			        NEXT FIELD nchq
			    END IF
			    #--------------------------------------------------#
			    ...
			    IF t1.tsdepcmon <> t3.cmon THEN
			        NEXT FIELD nchq
			    END IF
                            IF NOT f5040_buscar_adhuu_ts322(t3.uneg) THEN
                                LET g_msj = "Usuario no habilitado en la U/N ",
                                      t3.uneg USING "<<<<"
                                CALL qx030_msgadvetencia_gb000(g_msj)      
                                NEXT FIELD nchq
			...
			DISPLAY p1[l_curr].* TO s1[l_line].*
                ...
                BEFORE ROW
                        LET l_curr = arr_curr()
                        LET l_line = scr_line()
                        CALL f5030_buscar_tscha_ts322
				  (p1[l_curr].ncta,p1[l_curr].nchq)
				   RETURNING g_flag
...
FUNCTION f0410_repetidos_ts322(l_curr) 
        DEFINE l_curr  SMALLINT
	FOR i=1 TO arr_count()
            IF (i<>l_curr) AND (p1[i].ncta = p1[l_curr].ncta) AND
...
FUNCTION f0330_validar_ts322()
        ...
        END IF
	IF t1.tsdepiefe > 0 THEN
	    IF g_unec IS NULL OR g_unec <> g_unea THEN
                LET g_msj = "No puede revertir... La caja ha cambiado de U/N"
                RETURN FALSE
            END IF
	END IF
...
FUNCTION f4000_consulta_ts322()
        ...
        LET s1 = "SELECT * FROM tsdep WHERE ",query_1 CLIPPED,
                 " AND tsdeptdep = 1 AND tsdepmrcb = 0 ORDER BY tsdepntra"
        ...
        IF status = NOTFOUND THEN
            ...
            RETURN
        END IF
        CALL f6300_display_datos_ts322()
        MENU "CONSULTAR"
        COMMAND "Anterior" " "
...
FUNCTION f5000_buscar_registro_ts322()
        SELECT * INTO t1.*
                FROM  tsdep
...
FUNCTION f5010_buscar_gbcon_ts322(l_pfij,l_corr)
...
FUNCTION f5010_buscar_gbhtc_ts322(l_fech)
   	...
   	IF status <> 0 THEN
			...
			"Cambio para la fecha: ",l_fech USING "dd/mm/yyyy"
	    CALL qx030_msgadvetencia_gb000(g_msj)
            CLOSE q_gbhtc
            RETURN FALSE,0,0,0
   	END IF
...
FUNCTION f5020_buscar_tsccb_ts322()
...
FUNCTION f5030_buscar_tscha_ts322(l_ncta,l_nchq)
	DEFINE	l_ncta  LIKE tscha.tschancta,
	      	l_nchq	LIKE tscha.tschanchq
        INITIALIZE t3.* TO NULL
	SELECT tschaimpo,tschacmon,tschauneg,tschastat,tschamorg
	        INTO  t3.*
		FROM  tscha
	        WHERE tschacbco = t1.tsdepcbco
...
FUNCTION f5040_buscar_adhuu_ts322(l_uneg)
...
FUNCTION f5050_buscar_cnune_ts322(l_uneg)
...
FUNCTION f5100_buscar_detalle_ts322()
...
FUNCTION f5400_buscar_caja_ts322()
        INITIALIZE g_ncja,g_unec TO NULL
	SELECT adusrcaja INTO g_ncja
		FROM  adusr
		WHERE adusrusrn = g_user
	IF status = NOTFOUND THEN
	    LET g_ncja = NULL
	END IF
	SELECT cjmcjuneg INTO g_unec
		FROM  cjmcj
		WHERE cjmcjncja = g_ncja
		  AND cjmcjresp = g_user
	IF status = NOTFOUND THEN
	    LET g_ncja = NULL
	    LET g_unec = NULL
	END IF
...
FUNCTION f6000_limpiar_menu_ts322()
	IF g_mtflag = 0 THEN
	    CLEAR FORM
	END IF
        INITIALIZE t1.*,g_tdes,g_uncc,g_unea TO NULL
...
FUNCTION f6050_buscar_empresa_ts322()
...
FUNCTION f6300_display_datos_ts322()
	DEFINE l_desc  CHAR(5)
	IF g_mtflag = 0 THEN
            DISPLAY BY NAME t1.tsdepntra,t1.tsdepndoc,t1.tsdepftra,t1.tsdepcbco,
		            t1.tsdepncta,t1.tsdepcmon,t1.tsdeptcam,t1.tsdepiefe,
		            t1.tsdepgls1,t1.tsdepgls2,t1.tsdepgls3,t1.tsdepimpt,
		            t1.tsdepimpo
	END IF
	CALL f5010_buscar_gbcon_ts322(11,t1.tsdepcbco)
		  RETURNING g_bank,l_desc,g_flag
	CALL f5020_buscar_tsccb_ts322() RETURNING g_flag
	CALL f5010_buscar_gbcon_ts322(10,t1.tsdepmcta)
		  RETURNING g_dmct,g_amct,g_flag
	LET g_amct[5,5] = ":"
	CALL f5010_buscar_gbcon_ts322(10,t1.tsdepcmon)
		  RETURNING g_dmdp,g_amdp,g_flag
	...
	END IF
	CALL f5100_buscar_detalle_ts322()
	IF g_mtflag = 0 THEN
	    FOR i=1 TO 4 
	        IF i <= max_v1 THEN
	...
	END IF
	IF t1.tsdepiefe > 0 THEN
	    SELECT cjtrnntra,cjtrnncja,cjtrnuneg
		    INTO  g_tdes,g_acja,g_unea
		    FROM  cjtrn
		    WHERE cjtrnmorg = 96
		      AND cjtrntorg = t1.tsdepntra
	END IF
END FUNCTION
...
FUNCTION f7000_conv_importes_ts322(l_impi,l_cmon)
